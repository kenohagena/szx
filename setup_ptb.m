...
%[window, windowRect] = Screen('OpenWindow', screenNumber, 0.5);
[window, windowRect] = Screen('OpenWindow', screenNumber, 0.5, [1920, 0, 2*1920, 1080]);
...
load projector_calib_20160222
%gammaTables1 = repmat(gammaTables1, 1, 3);
...
options.kbqdev = [idx(strcmpi(names, 'ATEN Advance Tech Inc. VGA COMBO KVM V1.1.106')), idx(strcmpi(names, 'Current Designs, Inc. 932')),...
    idx(strcmpi(names, 'Apple Internal Keyboard / Trackpad')), idx(strcmpi(names, '')), idx(strcmpi(names, 'Dell WMI hotkeys')) ];
...
for i = 1:length(devices)
    if strcmp(devices(i).DeviceName,'ua25psychportaudio') %UA-25: USB Audio (hw:1,0)')
        break
    end
end
devices(i)
% check that we found the low-latency audio port
assert(numel(strfind(devices(i).DeviceName, 'ua25')) > 0, 'could not detect the right audio port! aborting')
audio = [];
